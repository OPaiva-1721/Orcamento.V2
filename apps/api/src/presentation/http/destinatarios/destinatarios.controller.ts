...
export class DestinatariosController {
  ...
  @Post()
  @HttpCode(201)
  create(@Body() body: CreateDestinatarioDto, @CurrentUser() user: DecodedFirebaseToken) {
    return this.createDestinatario.execute(body, user.uid);
  }

  @Get(':id')
  async findById(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: DecodedFirebaseToken) {
