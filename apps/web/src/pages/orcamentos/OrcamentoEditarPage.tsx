...
export function OrcamentoEditarPage() {
  const { id }   = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: orcamento, isLoading } = useOrcamento(Number(id));
  ...
  useEffect(() => {
    if (orcamento) {
      ...
      setDestinatarioIds((orcamento.destinatarios ?? []).map((d) => d.id));
      setDescricao(orcamento.descricao);
      setPreco(String(orcamento.preco));
      setStatus(orcamento.status);
      setFormaPagamento(orcamento.formaPagamento);
      setDataInicio(toDateInput(orcamento.dataInicio));
      setDataTermino(toDateInput(orcamento.dataTermino));
    }
  }, [orcamento]);
  ...
  function toggleDestinatario(did: number) {
    setDestinatarioIds((ids) => ids.includes(did) ? ids.filter((i) => i !== did) : [...ids, did]);
  }

  if (isLoading) return <div className="p-6 text-gray-500">Carregando...</div>;
  if (!orcamento) return <div className="p-6 text-red-500">Orçamento não encontrado.</div>;

  return (
    <div className="p-6 max-w-lg">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Editar Orçamento</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={updateOrcamento.isPending} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium disabled:opacity-50">
            {updateOrcamento.isPending ? 'Salvando...' : 'Salvar'}
          </button>
        </div>
      </form>
    </div>
